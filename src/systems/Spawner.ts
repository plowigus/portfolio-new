...
export class SpawnerSystem {
    ...
    public createPlatform(x: number, width: number): number {
        ...
        const y = GAME_CONFIG.height - 50; // Adjust based on visual preference or logic

        const graphics = new PIXI.Graphics();
        graphics.rect(0, 0, width, height);
        graphics.fill(0x555555);
        graphics.x = x;
        // graphics.y = y; // Original code sets y here
        // Wait, original code:
        ...
        this.platforms.push({ sprite: graphics, body: body });
    ...
    private createCoin(x: number, y: number) {
        ...
        const index = Math.max(0, this.app.stage.children.length - 1);
        this.app.stage.addChildAt(graphics, index); // Insert before score/debug if they are top
    ...
    private spawnObstacle() {
        ...
        // const platformVisualY = GAME_CONFIG.height - 50;
        ...
        const targetY = groundTopYReference + offset;
        ...
        graphics.x = startX;
        graphics.y = targetY;
        graphics.pivot.set(25, 25); // Pivot center
        ...
        Matter.World.add(this.engine.world, body);
        this.obstacles.push({ sprite: graphics, body: body });

        // spawn coins with obstacle
        if (isHighObstacle) {
            if (Math.random() > 0.5) {
                this.spawnCoinGroup(startX, groundTopYReference, GAME_CONFIG.coinSlideHeight, 'slide');
            } else {
